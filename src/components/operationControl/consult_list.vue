                                    ...
                                    <el-table :data="dataList" border style="width: 100%">
                                        <el-table-column label="属性">
                                            <template slot-scope="item">
                                                <span>{{item.row.fieldStatus == 0 ? '时效性' : item.row.fieldStatus == 1 ? '单次' : ""}}</span>
                                            </template>
                                        </el-table-column>
                    ...
                    <div class="panel-body">
                        <el-dialog :title="infoTitle" v-model="dialogVisible" size="small" >
                            <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="120px" onsubmit="return false">
                                ...
                                <el-form-item label="价格区间" prop="defaultPrice" >
                                    <el-row>
                                        <el-col :span="5">
...
<script>
    ...
    export default {
        ...
        data: function () {
            ...
            //     setTimeout(() => {
            //         if( !/^\d+(\.\d{2})?$/.test( value )) {
            //             callback(new Error('价格只能为浮点型，且只能为两位小数'));
            //         } else {
            //             callback();
            //         }
            //     }, 800);
            // };
            //开始或者结束的价格的验证规则，小数或者整数都符合要求；发现bug，ywl 2017年11月7日10:49:30进行更改
            var checkPrice = (rule, value, callback) => {
              if (!value) {
        ...
        methods: {
            ...
            modify: function(item){
                ...
                this.infoTitle = '修改（'+(item.status==0 ? '时效性' : '单次')+'咨询时长）';
            ...
            handleSave: function(formName){
                ...
                this.$refs[formName].validate(function(valid){
                    if (valid) {
                        _this.ruleForm.termOfValidity = (parseInt(_this.ruleForm.day)*24*60)+(parseInt(_this.ruleForm.hour)*60)+parseInt(_this.ruleForm.min);
                        _this.ruleForm.defaultPrice = _this.ruleForm.defaultPrice * 100; //价格以分为单位
                        _this.ruleForm.priceStart = _this.ruleForm.priceStart * 100;
                        _this.ruleForm.priceEnd = _this.ruleForm.priceEnd * 100;
                        _this.editLoading = true;
                        apiConsult['modify'](_this, _this.ruleForm).then(function (response) {
                            let body = response.body;
