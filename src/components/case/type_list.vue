                                ...
                                <div class="panel-body panel-body-rest">
                                    ...
                                    <el-table :data="dataList" border style="width: 100%">
                                        ...
                                        <el-table-column fixed="right" label="操作" width="300">
                                            <template slot-scope="item">
                                                <button title="编辑" v-if="item.row.state != 1" class="btn btn-primary btn-xs" @click="modify(item.row)"><i class="icon-pencil"></i>编辑</button>
                                                <button title="发布" v-if="item.row.state != 1" class="btn btn-info btn-xs" @click="updateState(item.row.id,1)"><i class="icon-check"></i>发布</button>
                                                <button title="停用" v-if="item.row.state != -1" class="btn btn-danger btn-xs" @click="updateState(item.row.id,-1)"><i class="icon-lock"></i>停用</button>
                                                <button title="删除" v-if="item.row.state != 1" class="btn btn-danger btn-xs" @click="confirm(item.row)"><i class="icon-trash"></i>删除</button>
                                            </template>
                                        </el-table-column>
                                    </el-table>
                                </div>
    ...
    export default {
        ...
        methods: {
            ...
            updateState: function(id,state){
                $("body").mLoading("show");
                apiCase.caseUpdateState(this, {id:id,state:state}).then(function (response) {
                    let body = response.body;
                    this.$message(body.msg);
