        ...
        <el-dialog :title="infoTitle" v-model="dialogVisible" size="tiny" >
            <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="120px" onsubmit="return false">
                ...
                <el-form-item label="关联案件类型" prop="parentId">
                    <el-select v-model="ruleForm.typeId" filterable placeholder="请选择" @change="getTask">
                        <el-option
...
<script>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                rules: {
                    ...
                    parentId:[
                        {required: true, message: '请选择关联案件类型', trigger: 'change'}
                    ]
                },
                options: [],
                optionsTask:[],
        ...
        methods: {
            init: function () {
                let _this = this;
                this.getCascade(0,function(opt){
                    _this.options = opt;
                });
            },
            getCascade: function(id,cb=function(opt){console.log(opt)}){
                apiCase.cascade(this, id).then(function (response) {
            ...
            getTask: function(id){
                let _this = this;
                this.getCascade(id,function(opt){
                    _this.optionsTask = opt;
                    _this.ruleForm.parentId = '';
                    if(_this.taskId != ''){
                        _this.ruleForm.parentId = _this.taskId ;
                        _this.taskId = '';
                    }
                });
            },
            ...
            modify: function(item){
                ...
                apiCase.getCase(this, item.id).then(function (response) {
                    let body = response.body;
                    if (body && body.code == 1) {
                        this.taskId = body.data.taskId+'';
                        if(this.ruleForm.typeId ==  body.data.typeId){
                            this.getTask(this.ruleForm.typeId);
                            return;
                        }
                        this.ruleForm.typeId = body.data.typeId+'';
                    }
                }, function (response) {
                    console.log('error:', response);
                });
